...
- 新建私有或公开仓库（建议**公开**，portfolio 本身是展示），推送现有历史。
- 推送时把默认分支从 `master` 更名为 `main`（一次性，与协作规则 §8 对齐）。

### 4.2 域名购买

- 目标域名：`sunzhonglun.com`（site.json 已按此配置）。
- 注册商建议 **Cloudflare Registrar**（成本价续费、免费 WHOIS 隐私、DNS 一并解决）；备选 Namecheap / Vercel Domains。
...
| 阶段 | 内容 | 验收 |
|------|------|------|
| 阶段 0 | 用户提供 92 篇 URL 清单（公众号后台「内容管理」导出，或逐篇复制链接），存 `docs/migration/urls.txt` | 清单齐、无重复 |
...
- `slug`：英文或拼音短语，小写连字符（如 `2019-03-02-du-shu-bi-ji`→ 更好是语义英文 `reading-notes`），迁移时逐篇定
...
- **侧栏**（桌面端 ≥ 1080px 左侧固定）：头像、一句话简介、导航、社交链接（GitHub / 邮箱 / 其他待定）、按年份的文章归档入口；窄屏折叠回顶栏
...
| # | 问题 | 阻塞哪个版本 |
|---|------|--------------|
| 1 | GitHub 账号 / 仓库公开还是私有？ | v1.0 |
| 2 | 域名注册商确认（建议 Cloudflare），用户自行完成购买 | v1.0 |
| 3 | 92 篇文章 URL 清单从哪来（公众号后台导出？） | v1.1 |
| 4 | 文章 slug 用英文语义还是拼音？ | v1.1 |
| 5 | 侧栏社交链接清单、头像图、一句话简介文案 | v1.2 |
| 6 | 精选文章挑哪几篇 | v1.2 |
