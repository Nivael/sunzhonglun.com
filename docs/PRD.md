...
- 有对应译文时，中英文页面右上角显示明确的 `中文 / EN` 双选项 tab；当前语言有清晰但克制的
  选中态。PC 端常驻页面右上，移动端独立换行并右对齐，不挤压主导航；没有译文的中文文章
  不显示虚假的语言入口。页面输出 `lang`、canonical 与双向 `hreflang`。
...
- [x] `中文 / EN` tab 在 PC 与移动端均无溢出、遮挡或导航挤压，键盘与读屏可辨认当前语言
