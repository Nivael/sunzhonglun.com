...
- v1.5 杂志化（首页/分类页，文章页不涉及）：报刊式 masthead（细线夹注元信息行 + 大字 hero + 粗细双线）；
  ...
  两栏网格（≥1360px 三栏，≤820px 单栏），封面 = 正文首图（`firstImage` 构建时推导，
  不加 frontmatter 字段），无图文章以 quote 排版块（.card-plate）代位
...
见 [docs/PRD.md](docs/PRD.md) §3 版本路线图。当前：**v1.5 杂志化改版**开发完成待验收
（见 PRD §7.6）；v1.4 访问统计已合并，线上数字待观察；随后 **v1.3 视频页**。

## 迭代记录

- **2026-07-22 v1.5 杂志化改版（开发完成，待验收合并）**：用户从四个候选参考中选定
